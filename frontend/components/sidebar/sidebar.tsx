...
export function Sidebar({ active, bookCounts, credits, onNavChange }: Props) {
  const pct = Math.min(100, Math.round((credits.remaining / credits.total) * 100))
      ...
      <div className={s.footer}>
        <div className={s.creditsBox}>
          <div className={s.creditsRow}>
            <span className={s.creditsLabel}>Credits remaining</span>
            <span className={s.creditsValue}>{credits.remaining} / {credits.total}</span>
          </div>
          <div className={s.creditsTrack}>
            <div className={s.creditsBar} style={{ width: `${pct}%` }} />
          </div>
          <button className={s.buyLink}>+ Buy more credits</button>
        </div>
      </div>
    </aside>
  )
